  ...
  return (
    <div className="space-y-4">
      ...
      <div className="print-page mx-auto max-w-[800px] bg-white text-black p-8 sm:p-12 rounded-lg border border-border shadow-sm">
        ...
        {/* Vehicle specs */}
        <div className="mb-6">
          <h3 className="font-bold text-sm uppercase tracking-wider mb-3 border-b border-gray-300 pb-1">
            Vehicle Description
          </h3>
          ...
          <div className="mb-6">
            ...
            <table className="w-full text-sm border border-gray-300">
              <thead className="bg-gray-100">
                <tr>
                  <th className="text-left px-3 py-2 font-medium border-b border-gray-300">#</th>
                  <th className="text-left px-3 py-2 font-medium border-b border-gray-300">Description</th>
                  <th className="text-right px-3 py-2 font-medium border-b border-gray-300">Amount (KES)</th>
                  <th className="text-left px-3 py-2 font-medium border-b border-gray-300">Due Date</th>
                </tr>
              </thead>
              <tbody>
                {loan.balance_items.map((item, idx) => (
                  <tr key={item.id} className="border-b border-gray-200">
                    <td className="px-3 py-2 font-mono-num">{idx + 1}</td>
                    <td className="px-3 py-2">{item.description}</td>
                    <td className="px-3 py-2 text-right font-mono-num">{item.amount.toLocaleString()}</td>
                    <td className="px-3 py-2 text-xs text-gray-600">{item.due_date ? formatDateLong(item.due_date) : '—'}</td>
                  </tr>
                ))}
                <tr className="bg-gray-100 font-bold">
                  <td colSpan={2} className="px-3 py-2 text-right">Total Balance:</td>
                  <td className="px-3 py-2 text-right font-mono-num">{loan.balance.toLocaleString()}</td>
                  <td></td>
                </tr>
              </tbody>
            </table>
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div>
            <p className="text-sm font-medium mb-8">Signed by the Seller:</p>
          ...
          <div>
            <p className="text-sm font-medium mb-8">Signed by the Buyer:</p>
            <div className="border-b border-black h-12 mb-1"></div>
            <p className="text-xs text-gray-600">{customer?.name || 'Buyer Name'}</p>
            <p className="text-xs text-gray-600 mt-1">Date: ____________________</p>
          </div>
        </div>
        ...
        {/* Witness */}
        <div className="mt-8">
          <p className="text-sm font-medium mb-8">In the presence of (Witness):</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <div className="border-b border-black h-12 mb-1"></div>
              <p className="text-xs text-gray-600">{loan.witness_name || 'Witness Name'}</p>
